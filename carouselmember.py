...
class CarouselMember(
	...
	def __init__(self,**kwargs):
		...
		kwargs["source"] = surati
		kwargs["keep_ratio"] = False
		kwargs["allow_stretch"] = True
		
		super().__init__(**kwargs)
		
		self.carousel_size = (1,1)

		self.current_width = 1
		
		self.size_hint = (1,self.carousel_size[1])
		self.pos_hint = {"center_x" :  0.5,"y" : 1-self.carousel_size[1]}
	...
	def hover(self,instance,cords):
		...
		if self.parent.members.index(self) == 0:
			...
			if x_cord > 0.5:
				self.pos_hint = {"x" : 0,"y" : 1-self.carousel_size[1]}
				self.current_width = 1 + (x_cord - 0.5)/20
				self.size_hint = (self.current_width,self.carousel_size[1])
			else:
				self.current_width = 1
				self.pos_hint = {"center_x" : x_cord,"y" : 1-self.carousel_size[1]}
			...
			if x_cord < 0.5:
				...
				self.pos_hint = {"x" : 1 - self.current_width ,"y" : 1-self.carousel_size[1]}
	...
	def hover_for_others(self,x_cord):
		...
		for m in self.parent.members:
			if m != self:
				...
				m.pos_hint = {"center_x" : x_cord + (self.parent.members.index(m) - self.parent.members.index(self)) ,"y" : 1-self.carousel_size[1]}
